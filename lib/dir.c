...
wambdir *wamb_opendirat(struct wamb *wamb, dev_t dev, ino_t ino)
{
	...
	struct wamb_child *child_list = (void *)val;
	size_t child_count = vall / sizeof(struct wamb_child);

	size_t i;

	
	struct wamb_child *child = child_list;
	for(i=0; i<child_count; i++) {
		wambdir_add_ent(dir, child->name, child->size, child->dev, child->ino);
		child++;
	}
